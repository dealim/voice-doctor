...
def multi_transcribe_audio(filename, extname):
    ...
    return response.results, encoding, sample_rate, filename, extname


def detect_language(multi_lan):
...
def transcribe_audio(filename, output_json_file, extname):
    multi_lan = multi_transcribe_audio(filename,extname)
    ...
    if (extname == 'speech.RecognitionConfig.AudioEncoding.LINEAR16'):
        encoding = 'LINEAR16'
    elif (extname == 'speech.RecognitionConfig.AudioEncoding.FLAC'):
        encoding = 'FLAC'
